...
from RL_Med.lib import plotting
...
featurizer = sklearn.pipeline.FeatureUnion([
        ("rbf1", RBFSampler(gamma=random.uniform(4.0,5.0), n_components=100)),
        ("rbf2", RBFSampler(gamma=random.uniform(1.0,2.0), n_components=100)),
        ("rbf3", RBFSampler(gamma=1.0, n_components=100)),
        ("rbf4", RBFSampler(gamma=random.uniform(0.5,1.0), n_components=100))
    ...
    for i_episode in range(num_episodes):
        ...
        if i_episode > 100:
            ...
            if np.mean(take) >= -110.0:
                ...
                flag = False
        '''
        f = open('history/Sarsa/Eps'+str(i_episode + 1)+'.txt', 'w')
