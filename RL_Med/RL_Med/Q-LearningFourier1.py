...
featurizer = sklearn.pipeline.FeatureUnion([
        ("f1", FourierSampler(constant=1.0, n_components=100)),
        ("f2", FourierSampler(constant=2.0, n_components=100)),
        ("f3", FourierSampler(constant=3.0, n_components=100)),
        ("f4", FourierSampler(constant=4.0, n_components=100)),
        ("f5", FourierSampler(constant=5.0, n_components=100)),
        #("f5", FourierSampler(constant=4.0, n_components=100)),
        #("f6", FourierSampler(constant=6.0, n_components=100)),
        #("f7", FourierSampler(constant=7.0, n_components=100)),
...
def q_learning(env, estimator, num_episodes, discount_factor=1.0, epsilon=0.0, epsilon_decay=1.0):
    ...
    for i_episode in range(num_episodes):
        ...
        '''
        if stats.episode_rewards[i_episode] >= -115.0:
            print("reached", count)
            count += 1
...
stats, estimator = q_learning(env, estimator, 10000, epsilon=0.0)
